
import { Link } from 'react-router-dom';
import { Button } from "@/components/ui/button";
import { useTranslation } from 'react-i18next';

interface BulkCategoryProps {
  name: string;
  image: string;
  moq: string;
  price: string;
}

export function BulkCategoryCard({ name, image, moq, price }: BulkCategoryProps) {
  const { t } = useTranslation();
  
  return (
    <div className="relative overflow-hidden rounded-lg shadow-sm group border border-gray-100">
      <img 
        src={image} 
        alt={name}
        className="w-full h-64 object-cover transition-transform duration-300 group-hover:scale-105"
      />
      <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent flex flex-col justify-end p-6">
        <h3 className="text-xl font-semibold text-white mb-2">{name}</h3>
        <p className="text-white/90 text-sm mb-1">MOQ: {moq}</p>
        <p className="text-white/90 text-sm mb-3">{price}</p>
        <Button 
          variant="outline" 
          size="sm" 
          className="bg-white/10 border-white/20 text-white hover:bg-white/20 backdrop-blur-sm w-full sm:w-auto"
          asChild
        >
          <Link to={`/chemicals?category=${name.toLowerCase()}&bulk=true`}>
            {t('viewDetails')}
          </Link>
        </Button>
      </div>
    </div>
  );
}
